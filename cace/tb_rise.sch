...
.control
   let vstart=\{Vdvdd\}*0.1
   let vstop=\{Vdvdd\}*0.9
   tran 0.5n 0.2m
   meas tran tstart WHEN v(dout)=vstart RISE=2
   meas tran tstop WHEN v(dout)=vstop TD=tstart RISE=1
   let Tr=tstop-tstart
   set wr_singlescale
   wrdata \{simpath\}/\{filename\}_\{N\}.data Tr
   quit
.endc
"}
...
* Find the 10%-90% rising edge time on 'dout'
