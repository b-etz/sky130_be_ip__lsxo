...
only_toplevel=true
value=".model DUMMY D()"}
C {devices/isource.sym} 110 -690 0 0 {name=Ibias value="DC \{Ibias\}"}
...
C {devices/vsource.sym} 240 -560 0 0 {name=Vdvss value="DC \{Vdvss\}" savecurrent=false}
C {devices/gnd.sym} 240 -530 0 0 {name=l3 lab=GND}
C {devices/vsource.sym} 240 -430 0 0 {name=Vavss value="DC \{Vavss\}" savecurrent=false}
...
save v(dout) v(x1.dout_ip) v(xin) v(xout) v(x1.x4.xin_buf)
tran 25n 0.3m
plot v(dout) v(x1.dout_ip) v(xin) v(xout) v(x1.x4.xin_buf)

*let veval=0.9*\{Vdvdd\}
*meas tran teval WHEN v(dout)=veval RISE=3
*let tstart=teval+5u
*let tend=teval+10u
*meas tran Voh MIN v(dout) from=tstart to=tend

*set wr_singlescale
*wrdata \{simpath\}/\{filename\}_\{N\}.data Voh
*quit
.endc
"}
...
* Find the current through the oscillator under condition of static input
...
spice_ignore=false
place=header}
C {devices/vsource.sym} 580 -170 0 0 {name=V1 value="SIN(0.5378 0.1027 32768 0 0 130)" savecurrent=false}
C {devices/vsource.sym} 540 -90 0 0 {name=V2 value="SIN(0.5450 0.0842 32768 0 0 0)" savecurrent=false}
